...
```
프로그램 
1. 데이터를 표현하고(자료구조) 2. 표현된 데이터를 처리하는 것(알고리즘)
...
for(idx = 0; idx < 10; idx++)
    sum += arr[idx];
```
:heavy_check_mark: 자료구조에 따라 알고리즘이 달라진다(알고리즘은 자료구조에 의존적)

# :grey_exclamation:알고리즘의 성능분석 방법
### 알고리즘을 평가하는 요소
* 시간 복잡도 : 입력의 크기와 문제해결 시간의 상관관계
...
데이터의 수가 많지 않고 성능에 덜 민감한 경우 알고리즘 B를 선택하기도 한다
```
> 상황에 맞게 판단해야한다<br>
###  순차 탐색 알고리즘
* 코드
...
순차 알고리즘의 데이터 수 n에 대한 연산횟수의 함수 : T(n) = n
```
### 이진 탐색 알고리즘
* 방식
```
